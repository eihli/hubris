...
impl ServerImpl {
    ...
    fn process_message(
        ...
        let (header, request) = decode_packet(&mut self.rx_buf)
            .map(|(header, request, _data)| (header, request))?;

        // If we successfully decoded a packet, reset tx_buf before potentially
        // encoding a response if needed.
        if reset_tx_buf {
            self.tx_buf.reset();
        }
...
fn decode_packet(
    rx_buf: &mut Vec<u8, MAX_PACKET_SIZE>,
    ...
    let (header, request, data) =
        host_sp_messages::deserialize::<HostToSp>(deframed).map_err(|err| {
            match err {
                HubpackError::Custom => DecodeFailureReason::Crc,
                _ => DecodeFailureReason::Deserialize,
            }
        })?;
